...
class Orchestrator:
    ...
    def run_steps(self, steps: List[Step], parameters):
        ...
        for step in steps_ordered:
            try:
                ...
                dependencies = [id_to_step[step_id]
                                for step_id in dependencies]

                _, validation_filename = self.generate_validation_file(
                    step, dependencies, parameters)
                success, message = self.validate_unit_code(validation_filename)

                for _ in range(Orchestrator.MAX_RETRIES):
                    if success:
                        break
                    code_snippet, _ = self.fix_step_source(
                        step, code_snippet, message)
                    success, message = self.validate_unit_code(
                        validation_filename)
                if not success:
                    continue
            except Exception as e:
                logger.error(
                    f"Error processing step {step.step_id}: {e}\n{traceback.format_exc()}\n")
                break
            # TODO: generate main
    ...
    def generate_step_file(self, step, parameters):
        ...
        print(
            f'generated step source for step {step.step_id}, filename: {filename}')
        print(code_snippet)
        print()
        return code_snippet, filename

    def generate_validation_file(self, step, dependencies, parameters):
        source = self.validation_generator.generate_source(
            step, dependencies, parameters)
        filename = f'validate_step_{step.step_id}.py'
        with open(self.working_dir + '/' + filename, 'w') as f:
            f.write(source)
        print(
            f'generated validation source for step {step.step_id}, filename: {filename}')
        print(source)
        print()
    ...
    def fix_step_source(self, step, code_snippet, error_message):
      ...
      # logger.info(f"promt for fixing: {prompt}")
      ...
      for line in lines:
          ...
          if "return" in line:
              break
      logger.info(f"source after fixing: {source}")
      filename = f'step_{step.step_id}.py'
      with open(self.working_dir + '/' + filename, 'w') as f:
          f.write(source)
      print(
          f'fixing step source for step {step.step_id}, filename: {filename}')
      print(source)
      print()
      return source, filename
